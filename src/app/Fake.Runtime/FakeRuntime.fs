...
let internal filterValidAssembly (logLevel: VerboseLevel) (isSdk, isReferenceAssembly, fi: FileInfo) =
    ...
    try
        let assembly = Mono.Cecil.AssemblyDefinition.ReadAssembly fullName
        ...
        { IsReferenceAssembly = isReferenceAssembly
          Info =
            { Runners.AssemblyInfo.FullName = assembly.Name.FullName
              ...
              Runners.AssemblyInfo.Location = fullName } }
        |> Some
    with e ->
        if logLevel.PrintVerbose then
            Trace.log <| sprintf "Could not load '%s': %O" fullName e
...
let paketCachingProvider
...
#else
    let framework =
        Paket.FrameworkIdentifier.DotNetFramework(Paket.FrameworkVersion.V4_6)
    ...
    let writeIntellisenseFile cacheDir =
        ...
        if logLevel.PrintVerbose then
            Trace.log <| sprintf "Writing '%s'" intellisenseFile
    ...
    let writeIntellisenseLazyFile cacheDir (context: Paket.LoadingScripts.ScriptGeneration.PaketContext) =
        ...
        async {
            try
                ...
                let groupScripts =
                    Paket.LoadingScripts.ScriptGeneration.generateScriptContent context
    ...
    let retrieveInfosUncached () =
        ...
        | None ->
            failwithf
                "The group '%s' was not found in the lockfile. You might need to run 'paket install' first!"
                groupName.Name
        ...
        let graph =
            async {
                if logLevel.PrintVerbose then
                    Trace.log <| sprintf "Calculating the runtime graph..."

                use runtimeGraphProfile =
                    Fake.Profile.startCategory Fake.Profile.Category.PaketRuntimeGraph
                    ...
                    |> Seq.choose (fun p ->
                        let r =
                            RuntimeGraph.getRuntimeGraphFromNugetCache
                                cacheDir
                                (Some PackagesFolderGroupConfig.NoPackagesFolder)
                                groupName
                                p.Resolved
        ...
        if logLevel.PrintVerbose then
            Trace.log <| sprintf "Retrieving the assemblies (rid: '%O')..." rid

        orderedGroup
        |> Seq.filter (fun p ->
            if p.Name.ToString() = "Microsoft.FSharp.Core.netcore" then
                eprintfn
                    "Ignoring 'Microsoft.FSharp.Core.netcore' please tell the package authors to fix their package and reference 'FSharp.Core' instead."
        ...
        |> Seq.map (fun p ->
            async {
                ...
                | Some installModelTask ->
                    ...
                    let installModel =
                        orig.ApplyFrameworkRestrictions(
        ...
        |> Seq.groupBy (function
            | DependencyFile.Assembly ass ->
                ass.IsReferenceAssembly, System.Reflection.AssemblyName(ass.Info.FullName).Name
    ...
    let restoreOrUpdate () =
        ...
        if
            needLocalLock
            && File.Exists localLock
            && (not (File.Exists lockFilePath.FullName)
                || localLockText.Value <> File.ReadAllText lockFilePath.FullName)
        then
        ...
        if not <| File.Exists lockFilePath.FullName then
            if logLevel.PrintVerbose then
                Trace.log "Lockfile was not found. We will update the dependencies and write our own..."

            try
                paketApi.UpdateGroup(
                    groupStr,
                    false,
                    false,
                    false,
                    false,
                    false,
                    Paket.SemVerUpdateMode.NoRestriction,
                    false
                )
                |> ignore
            with e when e.Message.Contains "Did you restore group" ->
            ...
            if needLocalLock then
                File.Copy(lockFilePath.FullName, localLock, true)

        // Restore
        if config.RestoreOnlyGroup then
            if config.UseSimpleRestore then
                RestoreProcess.Restore(
                    paketApi.DependenciesFile,
                    RestoreProcess.RestoreProjectOptions.NoProjects,
                    false,
                    Option.map Paket.Domain.GroupName group,
                    true,
                    false,
                    None,
                    None,
        ...
        else if config.UseSimpleRestore then
            paketApi.SimplePackagesRestore()
        else
            paketApi.Restore()
    ...
    let readFromCache () =
        ...
        |> Seq.map (fun line ->
            ...
            | Assembly isRef ->
                ...
                let loc =
                    Path.readPathFromCache config.ScriptFilePath (splits.[2].Replace("%20", " "))
    ...
    let writeToCache (list: DependencyFile list) =
        ...
        |> Seq.map (function
            | DependencyFile.Assembly item ->
                sprintf
                    "%s;%s;%s;%s"
                    (if item.IsReferenceAssembly then
                         "REFERENCEASSEMBLY"
                     else
                         "RUNTIMEASSEMBLY")
                    item.Info.Version
    ...
    let getKnownDependencies () =
        CoreCache.getCached
            (fun () ->
                let list = retrieveInfosUncached () in
                writeIntellisenseTask.Value |> ignore
                list)
            readFromCache
            writeToCache
            (fun _ ->
                File.Exists dependencyCacheHashFile
                && File.Exists dependencyCacheFile
                && File.ReadAllText dependencyCacheHashFile = File.ReadAllText lockFilePath.FullName)
        ...
        Trace.tracefn
            ...
            (System.String.Join(
                ...
                |> Seq.map (fun d ->
                    let typ, ver =
                        match d with
                        | DependencyFile.Assembly a ->
                            (if a.IsReferenceAssembly then "ref" else "lib"), sprintf " (%s)" a.Info.Version
    ...
    { new CoreCache.ICachingProvider with
        member x.CleanCache context =
            ...
            with e ->
                Trace.traceError (sprintf "Failed to delete cached files: %O" e)

        member __.TryLoadCache(context) =
            let references =
                knownDependencies.Value
                |> List.choose (function
                    | DependencyFile.Assembly a when a.IsReferenceAssembly -> Some a
                    | _ -> None)
            ...
            let runtimeAssemblies =
                knownDependencies.Value
                |> List.choose (function
                    | DependencyFile.Assembly a when not a.IsReferenceAssembly -> Some a
                    | _ -> None)
            ...
            let nativeLibraries =
                knownDependencies.Value
                |> List.choose (function
                    | DependencyFile.Library a -> Some a
                    | _ -> None)
            ...
            if File.Exists(assemblyPath) && File.Exists(warningsFile) then
                Some
                    { CompiledAssembly = assemblyPath
                      Warnings = File.ReadAllText(warningsFile) }
            else
                None

        member x.SaveCache(context, cache) =
            if logLevel.PrintVerbose then
                Trace.log "saving cache..."
...
let internal tryFindGroupFromDepsFile scriptDir =
    ...
    if File.Exists(depsFile) then
        ...
        | Some group ->
            ...
            FakeHeader.PaketDependencies(
                FakeHeader.PaketDependenciesRef,
...
let tryPrepareFakeScript (config: FakeConfig) : TryPrepareInfo =
    ...
    let inline getSectionUncached () =
        ...
        let newSection =
            FakeHeader.tryReadPaketDependenciesFromScript config.ScriptTokens.Value cacheDir script

        match newSection with
        | Some s -> Some s
        | None -> tryFindGroupFromDepsFile scriptDir
    ...
    let writeToCache (section: FakeHeader.FakeSection option) =
        let content =
            ...
            | Some (FakeHeader.PaketDependencies (headerType, p, _, group)) ->
                ...
                sprintf
                    "paket: %s, %s, %s"
                    (Path.fixPathForCache config.ScriptFilePath p.DependenciesFile)
                    (match group with
                     | Some g -> g
                     | _ -> "<null>")
                    s
    ...
    let section =
        CoreCache.getCached getSectionUncached readFromCache writeToCache (fun _ ->
            File.Exists scriptSectionHashFile
            && File.Exists scriptSectionCacheFile
            && File.ReadAllText scriptSectionHashFile = File.ReadAllText script)
...
let prepareFakeScript (config: FakeConfig) : PrepareInfo =
    ...
    | NoHeader (cacheDir, saveCache) ->
        ...
        storage: none
        framework: netstandard2.1,netstandard2.0,net6.0,net8.0
        nuget FSharp.Core
                """
...
let createConfig
    ...
    if logLevel.PrintVerbose then
        Trace.log (
            sprintf
                "prepareAndRunScriptRedirect(Script: %s, fsiOptions: %A)"
                scriptPath
                (System.String.Join(" ", fsiOptions))
        )
    ...
    let tokenized =
        lazy
            (File.ReadLines scriptPath
             |> FSharpParser.getTokenized scriptPath ("FAKE_DEPENDENCIES" :: newFsiOptions.Defines))
